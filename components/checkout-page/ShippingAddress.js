...
function ShippingAddress({ addressComplete, updateCompleteness,setActive }) {
    ...
    const updateZipcode = async (zip) => {
        ...
        } else updateAddressValue("zip_code", zip);
    };
    ...
    const checkAndSave = () => {
        ...
        if (dataStore.userData.contact && selectedAddressIndex && editAddress === false) {
            completeness = true;
        } else {
            completeness = addressCompleteness() && extraMeasureCompleteness() && createAccount;
        }
        ...
        if (completeness !== addressComplete) updateCompleteness(completeness);
    ...
    const addressIndex = (index, edit) => {
        ...
        if (edit !== editAddress) setEditAddress(edit);
    };

    useEffect(() => {
        console.log("I WAS TRIGGERED");
        if (dataStore.userData.contact && dataStore.userAddresses.length > 0 && address.address === "")
    ...
    console.log(
        "editAddress, dataStore.userData.contact, dataStore.userAddresses.length",
        editAddress,
        dataStore.userData.contact,
        dataStore.userAddresses.length,
    );
    ...
    const mobileView = (
        <Fragment>
            ...
            {true ? (
                ...
                <Fragment>
                    <div className='px-8 grid grid-cols-1 text-[#555] pb-20'>
                        <div>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e) => updateBDay("birthday", e.target.value)}
                                value={bDay.birthday}
                            />
                        </div>
                        <div>
                            ...
                            <input
                                ...
                                type='date'
                                min={DateTime.now().minus({ year: 18 }).toISODate()}
                                onChange={(e) => updateBDay("anniversary", e.target.value)}
                                value={bDay.anniversary}
                            />
                        </div>
                        <div>
                    ...
                    <Fragment>
                        ...
                        <div className='grid grid-cols-2 gap-x-8 gap-y-4 text-[#555]'>
                            ...
                            <div className='col-span-full flex justify-center gap-10 text-sm'>
                                ...
                                {addressSaved ? <span> saved </span> : null}
                            </div>
                        </div>
                        <Toast show={show} hideToast={() => setShow(false)}>
                            <span>{message}</span>
